...
const Auth = Router()
    .get("/login", async (req, res) => {
        if (req.query.code) {
            ...
            params.set(
                "redirect_uri",
                `${req.dashboardConfig.baseUrl}/auth/login`
            );
            ...
            if (tokens.error || tokens.access_token == undefined || tokens.access_token == null) {
                console.log(tokens);
            ...
            res.redirect(`https://discord.com/api/oauth2/authorize?client_id=${req.client?.user?.id}&scope=identify%20guilds&response_type=code&redirect_uri=${encodeURIComponent(`${req.dashboardConfig.baseUrl}/auth/login`)}`);
        }
    })
