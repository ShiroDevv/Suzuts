...
bot.on("message", async (message) => {
    if (message.toString().includes("You're currently guild muted for"))
        return bot.chat('/g unmute ' + bot.username);
...
client.on("messageCreate", async (message) => {
    ...
    if (message.channel.id == "878026030877663262") {
        ...
        if (filter.isUnclean(message.content))
            return message.reply("Screw you, nice try, I added a content filter");
        if (filter.isUnclean(message.content.replaceAll(" ", "")))
            return message.reply("Screw you, nice try, I added a content filter");
