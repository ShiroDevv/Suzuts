...
export const playMessage: Commands = {
    name: "play",
    ...
    run: async function (client: any, message: any, args: Array<string>) {
        ...
        if (!args[1]) return message.reply("Missing song title");
        if (!message.member.voice.channel) return message.reply({ content: "Please join a voice channel first!", emphameral: true })
