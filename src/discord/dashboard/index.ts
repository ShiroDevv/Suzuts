...
export default class Dashboard extends EventEmitter {
    ...
    constructor(client : Client, options : any) {
        ...
        if(+process.versions.node.split(".") < 16) throw new Error("Dashboard need node-16+");
        ...
        if(!this.config.secret) console.warn("Without the client.secret paremeter, some features of the dashboard will be disabled, like Discord authetification or guild settings...");
    ...
    _getTheme(theme : any) {
        if(!theme) require(join(__dirname, "themes", "dark") + "/index.cjs");
    ...
    _setup() {
        ...
        this.app.use((req : any, res, next) => {
            ...
            req.dashboardEmit = (...args : any) => this.emit("", ...args);
    ...
    _checkRoutes() {
        ...
        for (let routeFile of readdirSync(join(__dirname, "routes")).filter((e) =>
            ...
            if (["auth.cjs", "custom.cjs"].includes(routeFile)) continue;
    ...
    registerCommand(name : string, description : string, usage : Array<string>) {
        this._commands.push({ name, description, usage });
    }

    addTextInput(name : string, description : string, validator: (value: any) => boolean, setter: (client: Client, guild: Guild, value: string) => void, getter: (client: Client, guild: Guild) => string | Promise<string>) {
        this._settings.push({
            name,
            description,
            type: "text input",
    ...
    addBooleanInput(name : string, description : string, setter: (client: Client, guild: Guild, value: boolean) => void, getter: (client: Client, guild: Guild) => boolean | Promise<boolean>) {
        this._settings.push({ name, description, type: "boolean input", set: setter, get: getter });
    }

    addColorInput(name: string,description: string,setter: (client: Client, guild: Guild, value: string) => void,
    ...
    addSelector(name : string, description : string, getSelectorEntries: (client: Client,guild: Guild) => [string, string][] | Promise<[string, string][]>, setter: (client: Client, guild: Guild, value: boolean) => void, getter: (client: Client, guild: Guild) => boolean | Promise<boolean>) {
        this._settings.push({
            name,
            description,
            type: "selector",
