...
export default async function maroNetworth(username: string) {
...
export async function getProfile(uuid: string, apiKey: string | undefined) {
    ...
    if (typeof (response) == "string") return "Error getting data from hypixel api";
    const { data } = response;

    if (!data?.profiles) return "Error: Couldn't get profileData for this player.";
...
export function getActiveProfile(profiles: any, uuid: string) {
    return profiles.sort((a: any, b: any) => b.members[uuid].last_save - a.members[uuid].last_save)[0];
}

export async function getNetworth(profile: any, returnCategories: boolean | undefined = false) {
    if (!profile || profile == "{}") throw new Error("Missing profile");
    ...
    if (typeof (response) == 'string') return "Error getting data from maro api";
