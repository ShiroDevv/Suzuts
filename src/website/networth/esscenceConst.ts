...
export default async function getEsscenceCost(itemID: string, starAmount: number) {
    if (starAmount == 0) return 0;
    const json = fs.readFileSync(`${process.cwd().replaceAll("\\", "/")}/sbJsonFiles/constants/essencecosts.json`);
    const stringJson: any = json.toString();
    ...
    const ItemJson: any = esscenseJson[itemID];

    if (!ItemJson) return 0;

    let EsscenceType: string = "";

    let esscenceAmount: number = 0;

    let starLoops = 0;

    for (let key in ItemJson) {
        if (key == "type") EsscenceType = `ESSENCE_${ItemJson[key].toUpperCase()}`;

        key = key.replace(/[^1234567890]/g, "");

        if (+key == 0 || +key == null || key == undefined) continue;

        if (+key > starAmount) continue;
    ...
    let essencecost: number = 0;

    if (typeof (apiResponse) != "number") {
        if (EsscenceType == "") return 0;
    ...
    if(starLoops != starAmount) console.error("ERROR: STARLOOPS WRONG");
