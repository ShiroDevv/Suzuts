        ...
        (err, files) => {
            for (var i = 0; i < files.length; i++) {
                var dirname = files[i].replace(
                    "C:/Users/ayvie/Desktop/Suzu.ts/website/routes/extraPages/docs",
                    ""
                );
                functionArray[i] = (name : string) => {;
                    app.use("/docs" + name.replace(".html", ""), (req: any, res: any) => {
                        res.sendFile(
            ...
            app.use("/gameEngine", async(req : any, res : any) => {
                res.sendFile(
                    path.join(
                        process.cwd().toString().replaceAll("\\", "/") +
            ...
            app.use("/games/basicPlaformer", async (req : any, res : any) => {
                res.sendFile(
                    path.join(
                        process.cwd().toString().replaceAll("\\", "/") +
